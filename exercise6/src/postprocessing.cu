...
__global__ void antialiasepixel(rgb* result, int width, int height)
{
    ...
    if(x < width && y < height)
    {
        result[y*width+x] = mix(tex2D(textureImage, x, y),
...
void antialiase(int height, int width, rgb* image)
{
    ...
    rgb *d_result;
    error = cudaMalloc(&d_result, width*height*sizeof(rgb) );
    CHECK_EQ(cudaSuccess, error) << "Error at line "<< __LINE__ << ": " << cudaGetErrorString(error);
    ...
    //copy back result
    error = cudaMemcpy(image, d_result, width*height*sizeof(rgb), cudaMemcpyDeviceToHost);
    CHECK_EQ(cudaSuccess, error) << "Error at line "<< __LINE__ << ": " << cudaGetErrorString(error);
