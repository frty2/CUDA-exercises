...
void initial_ray(const camera& c, const point& upperleft, int x, int y, point& xgap, point& ygap, ray& r)
{
    //place the ray in the middle of the hole (not top left)
    point p = upperleft + (x+0.5) * xgap - (y+0.5) * ygap;
    r.location = p;
...
void render_image(const scene& s, const int& height, const int& width, rgb* image)
{
    ...
    for(y = 0; y < height; y++)
    {
        ...
        {
            ray r;
            initial_ray(s.cam, upperleft, x, y, xgap, ygap, r);
